...
from salvacion.env_tools import (
    ACTION_MAP,
    build_state,
    scan_doors,
    shortest_path_length,
)
from salvacion.expert_planner import ACTION_INDEX, action_from_direction, compute_path_info
from salvacion.evaluate import evaluate_agent
...
def default_log_dir() -> pathlib.Path:
    ...
    return pathlib.Path("logs") / f"salvacion_qlearning_{timestamp}"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train salvacion Q-learning agent")
    parser.add_argument(
        "--config",
        type=str,
